...
switch($act){
	...
	case 'create_submit':
		...
		if(empty($title)) ShowError('项目名称不能为空',URL_ROOT.'/index.php?do=project&act=create');
		$db=DBConnect();
		//生成短网址字符
			...
			foreach($modules as $mId){
				...
				if(!empty($module) && !empty($module['setkeys'])){
					...
					foreach($mSetKeys as $setkey){
						...
						if(!empty($setkeyV)) $moduleSetKeys["$setkeyK"]=urlencode($setkeyV);
					}
				}
			}
	...
	case 'viewcode':
		...
		$smarty->assign('shortShow2',$shortShow2);
